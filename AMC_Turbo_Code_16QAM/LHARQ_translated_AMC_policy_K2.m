...
%epsilon_Set=[logspace(-6,-1.1,25)  linspace(0.08,0.99,25)];
%epsilon_Set=[logspace(-2,-1.1,10)  linspace(0.08,0.99,10)];
tic 
epsilon_Set=[logspace(-2,-1.1,10)  linspace(0.08,0.99,10)];
...
for ind_snr=24 %1:1:length(SNRdB)
    ...
    delta_set=1 ;%logspace(0,log10(snr_range(ind_snr,end-1)./snr_range(ind_snr,2)),20);
    
    for ind_delta=1:length(delta_set)
       [ind_snr ind_delta]
        snr_mm=snr_m./delta_set(ind_delta);
        
        for ind_epsilon=9 %1:length(epsilon_Set)
            ind_epsilon
            counter=0;
            reward=0;
            number_ack_1=0;
            while counter<N_realization-1
                ...
                wep_round_1=WEP_function(rate_round_1,0,gamma(counter));
                ack_round_1=binornd(1, 1-wep_round_1);
                    if ack_round_1==1
                        ...
                        if BK_rate_round_1<=rate_round_2
                            BK_wep_round_1=WEP_function(rate_round_1,BK_rate_round_1,gamma(counter));
                            BK_wep_round_1=min(max(BK_wep_round_1/wep_round_1,0),1);
                            BK_ack_round_1=binornd(1, 1-BK_wep_round_1);
                            wep_round_2=WEP_function(rate_round_2,0,gamma(counter+1));
            ...
            throughput_lharq_epsilon(ind_epsilon)= reward/counter;
        end
        [throughput_lharq_delta(ind_delta,ind_snr),ii]=max(throughput_lharq_epsilon);
...
%save LHARQ_translated_AMC_policy_16QAM_turbo_code_dop_0.05rho_0.95156_final.mat  dop rho SNRdB throughput_lharq_delta optimal_epsilon_delta...
