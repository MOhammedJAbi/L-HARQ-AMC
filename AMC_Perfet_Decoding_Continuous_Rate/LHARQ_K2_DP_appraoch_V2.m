...
for ind_snr=1:length(SNRdB)
    ...
    snr_m=x';
    snr_m=[0 snr_m 1e100];
    [Mat_need2,Mat_snr]=meshgrid(need_set,snr_m');
    ...
    for ind_lambda=1:length(lambda_set)
        ...
        V_2=@(ge2,R2,n) (R2-n).*(R2>=n).*marcumq(sqrt(rho*2.*ge2./((1-rho).*snr)),sqrt(2.*(2.^R2-1)./((1-rho).*snr)))-lambda.*(R2>=n);
        ...
        for i=1:length(need_set);
            [val0,ind]=max(V_2(Mat_g,Mat_rate,need_set(i))');
            Parity_Rate2(i,:)=rate_set(ind);
            for j=1:length(rate_set)
                val=V_2c(x,rate_set(ind)',rate_set(j),need_set(i));
                xtemp=val.*pdf_snr(x);
        ...
        policyInterpolation = scatteredInterpolant(Mat_need2(:),Mat_snr(:),MParity_Rate2(:));
...
save LHARQ_AMC_DP_policy_Perfect_Decoding_continuous_R_dop_0.1_rho_0.8_V2.mat SNRdB lambda_set optimal_lambda_lharq rho policy_lharq throughput_lharq
