...
f_threshold1=@(x,R,av_snr,epsilon) (1-marcumq(sqrt(rho*2.*x./((1-rho).*av_snr)),sqrt(2.*Qam_th(R)./((1-rho).*av_snr))))./epsilon-1;

% Legend integral
lint =2e2;
[x0, w] = GaussLegendre(lint);

epsilon_set=logspace(-1.7,-0.01,50)
N=1e6;
K=4;
...
for ind_snr=1:length(SNRdB)
    ...
    for ind_epsilon=1:length(epsilon_set)
        for j=1:length(x)
            policy_tem(j)=bisection(@(R) f_threshold1(x(j),R,snr,epsilon_set(ind_epsilon)) ,1e-5,4, 2e2, 1e-5, 1e-2);
            policy_tem(j)=min(policy_tem(j),R_max);
        end
    ...
    for j=1:length(x)
            policy_tem(j)=bisection(@(R) f_threshold1(x(j),R,snr,epsilon_set(I)) ,1e-5,4, 2e2, 1e-5, 1e-2);
            policy_tem(j)=min(policy_tem(j),R_max);
     end
...
save LHARQ_outage_policy_16QAM_Perfect_Decoding_continuous_R_dop_0.1_rho_0.8_V_leger_V4.mat dop rho SNRdB snr_range ...
