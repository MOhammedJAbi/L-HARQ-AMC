...
for ind_snr=36:length(SNRdB)
    ...
    for ind_epsilon=1:length(epsilon_set)
        ind_epsilon
        for j=1:length(x)
            policy_tem(j)=bisection(@(R) f_threshold1(x(j),R,snr,epsilon_set(ind_epsilon)) ,1e-5,4, 1e2, 1e-5, 1e-2);
        end
    ...
    for j=1:length(x)
            policy_tem(j)=bisection(@(R) f_threshold1(x(j),R,snr,epsilon_set(I)) ,1e-5,4, 1e2, 1e-5, 1e-2);
     end
...
save LHARQ_outage_policy_16QAM_Perfect_Decoding_continuous_R_dop_0.05_rho_0.95_V_leger_V3.mat dop rho SNRdB snr_range ...
